...
# Kiwi 형태소 분석기 초기화 및 사용자 정의 명사 추가
def initialize_kiwi():
...
stopwords = {
    ...
    '동', '데', '국', '밖', '속', '식', '융', '밖', '규', '현행법', '직', '범', '만', '입', '신',
    ...
    '더라도', '더불어', '동시에', '된바에야', '된이상', '두번째로', '둘', '등등', '따라', '따지지 않다',
    '때가 되어', '또', '또한', '라 해도', '령', '로', '로 인하여', '로부터', '를', '마음대로', '마저',
    '막론하고', '만 못하다', '만약', '만약에', '만은 아니다', '만일', '만큼', '말하자면', '매', '모두',
    '무렵', '무슨', '물론', '및', '바꾸어말하면', '바꾸어서 말하면', '바로', '밖에 안된다', '반대로',
    '반드시', '버금', '보는데서', '본대로', '부류의 사람들', '불구하고', '사', '삼', '상대적으로 말하자면',
    '설령', '설마', '셋', '소생', '수', '습니까', '습니다', '시간', '시작하여', '실로', '심지어', '아',
    '아니', '아니라면', '아니면', '아야', '아울러', '안 그러면', '않기 위하여', '알 수 있다', '앞에서',
    '약간', '양자', '어', '어느', '어디', '어때', '어떠한', '어떤', '어떻게', '어째서', '어쨋든', '언제',
    '얼마', '얼마만큼', '엉엉', '에', '에게', '에서', '에이', '엔', '영', '예', '올', '와', '와르르', '와아',
    '왜', '외에', '요', '우리', '원', '월', '위에서 서술한바와같이', '윙윙', '육', '으로', '으로서', '으로써',
    '을', '응', '응당', '의', '의거하여', '의지하여', '의해', '이', '이르다', '이쪽', '인젠', '일', '일것이다',
    '임에 틀림없다', '자기', '자기집', '자마자', '자신', '잠깐', '잠시', '저', '저기', '저쪽', '전부', '전자',
    '정도에 이르다', '제', '제외하고', '조차', '좋아', '좍좍', '주', '주저하지 않고', '줄', '중이다', '즈음',
    '즉', '지금', '지말고', '진짜로', '쪽으로', '쯤', '차라리', '참', '첫번째로', '쳇', '콸콸', '쾅쾅', '쿵',
    '큼', '타다', '통하다', '틈타', '팍', '퍽', '하', '하게될것이다', '하게하다', '하겠는가', '하고', '하고있었다',
    '하곤하였다', '하구나', '하기 때문에', '하기만 하면', '하기보다는', '하기에', '하나', '하느니', '하는 것만',
    '하는 편이 낫다', '하는것도', '하더라도', '하도다', '하도록시키다', '하도록하다', '하든지', '하려고하다',
    '하마터면', '하면 할수록', '하면서', '하물며', '하여금', '하여야', '하지 않는다면', '하지 않도록', '하지마',
    '하지마라', '하천', '하품', '한', '한 이유는', '한 후', '한다면', '한다면 몰라도', '한데', '한마디',
    '한적이있다', '한켠으로는', '한편', '할 따름이다', '할 생각이다', '할 줄 안다', '할 지경이다', '할때',
    '할만하다', '할망정', '할뿐', '함께', '해도', '해봐요', '해서는', '해야한다', '해요', '했어요',
    '형식으로 쓰여', '혹시', '혹은', '혼자', '훨씬', '휘익', '휴', '흐흐', '힘입어',
...
def prepare_coherence(df):
    texts = [doc.split() for doc in df['content'] if doc]
    dictionary = Dictionary(texts)
    corpus = [dictionary.doc2bow(text) for text in texts]
    print(f"🔍 Corpus 생성 완료: {len(corpus)} 문서")
    return texts, dictionary, corpus

# 엘보우 기법으로 최적 클�러스터 수 결정
def find_optimal_n_topics(X, texts, dictionary, corpus, min_topics=10, max_topics=50, step=5):
    perplexities = []
    coherences = []
    ...
    if len(perplexities) < 3:
        ...
        max_coherence_idx = np.argmax(coherences)
        if coherences[max_coherence_idx] > coherences[elbow_idx] + 0.05:
            best_n_topics = n_topics_range[max_coherence_idx]
...
def refine_keywords_with_gemini_cached(keywords_tuple, model):
...
'''
            ...
            if response.text:
                ...
                for kw in kw_list:
                    if (kw in stopwords or kw in excluded_terms or kw in excluded_bigrams or
                        ...
                        any(kw in existing for existing in filtered_keywords) or
                        kw in {'식품', '어업', '농업', '수산'}):
                        continue
                ...
                for kw in keywords:
                    if (kw not in filtered_keywords and
                        ...
                        not any(kw in existing for existing in filtered_keywords) and
                        kw not in {'식품', '어업', '농업', '수산'}):
                        filtered_keywords.append(kw)
                    if len(filtered_keywords) >= 4:
                        break
                if len(filtered_keywords) >= 4:
    ...
    extra_keywords = [
        ...
        not any(part in stopwords or part in excluded_terms for part in feature_names[i].split()) and
        feature_names[i] not in {'식품', '어업', '농업', '수산'}
    ]
    filtered_keywords.extend(extra_keywords)
    return ", ".join(filtered_keywords[:4])

def refine_keywords_parallel(topic_keywords, model):
    topic_labels_refined = {}
    with ThreadPoolExecutor(max_workers=1) as executor:  # 병렬 부하 최소화
        future_to_topic = {
            executor.submit(
...
def main():
    ...
    # 데이터 전처리
    df['content'] = extract_nouns(df['content'].tolist(), kiwi)
    ...
    # Coherence 준비
    texts, dictionary, corpus = prepare_coherence(df)

    # 최적 클러스터 수 계산
    n_topics = find_optimal_n_topics(X, texts, dictionary, corpus, min_topics=10, max_topics=50, step=5)

    # LDA 학습
    lda = train_lda(X, n_topics)
    ...
    topic_labels_refined = refine_keywords_parallel(topic_keywords, model)
    ...
    df['topic_label'] = df['topic_label'].fillna(", ".join(feature_names[:4]))
