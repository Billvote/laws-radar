...
custom_nouns = [
    ...
    '국민権益위원회', '금융위원회', '개인정보보호위원회', '원자력안전위원회', '기획재정부',
...
excluded_terms = {
    ...
    "국민권익위원회 권고안", "벌금액",
    "산업통상자원부", "산업통상자원부 장관", "중소벤처기업부", "중소벤처기업부 장관",
    "대통령비서실", "국가안보실", "대통령경호처", "국가안전보장회의", "민주평화통일자문회의",
    "국민경제자문회의", "국가과학기술자문회의", "감사원", "국가정보원", "방송통신위원회",
    "특별감찰관", "고위공직자범죄수사처", "국가인권위원회", "국무조정실", "국무총리비서실",
    "인사혁신처", "법제처", "식품의약품안전처", "공정거래위원회", "국민권익위원회", "금융위원회",
    "개인정보보호위원회", "원자력안전위원회", "기획재정부", "국세청", "관세청", "조달청", "통계청",
    "교육부", "과학기술정보통신부", "우주항공청", "외교부", "재외동포청", "통일부", "법무부", "검찰청",
    "국방부", "병무청", "방위사업청", "행정안전부", "경찰청", "소방청", "국가보훈부", "문화체육관광부",
    "국가유산청", "농림축산식품부", "농촌진흥청", "산림청", "산업통상자원부", "특허청", "보건복지부",
    "질병관리청", "환경부", "기상청", "고용노동부", "여성가족부", "국토교통부", "행정중심복합도시건설청",
    "새만금개발청", "해양수산부", "해양경찰청", "중소벤처기업부",
    "기재부", "교육부", "과기정통부", "외교부", "통일부", "법무부", "국방부", "행안부", "문체부",
    "농식품부", "산업부", "복지부", "환경부", "고용부", "여가부", "국토부", "해수부", "중기부",
    "대통령", "국무총리", "국무위원", "장관", "처장", "위원장", "청장", "실장", "부총리",
    "기획재정부 장관", "교육부 장관", "과학기술정보통신부 장관", "외교부 장관", "통일부 장관",
    "법무부 장관", "국방부 장관", "행정안전부 장관", "문화체육관광부 장관", "농림축산식품부 장관",
    "산업통상자원부 장관", "보건복지부 장관", "환경부 장관", "고용노동부 장관", "여성가족부 장관",
    "국토교통부 장관", "해양수산부 장관", "중소벤처기업부 장관", "국가보훈부 장관",
    "식품의약품안전처장", "공정거래위원장", "금융위원장", "원자력안전위원장", "개인정보보호위원장",
    "방송통신위원장", "국가인권위원장", "국민권익위원장", "병무청장", "경찰청장", "소방청장",
    "해양경찰청장", "관세청장", "조달청장", "통계청장", "국세청장", "특허청장", "산림청장",
    "기상청장", "농촌진흥청장", "재외동포청장", "우주항공청장", "국가유산청장", "방위사업청장",
    "행정중심복합도시건설청장", "새만금개발청장",
    "기재부 장관", "과기정통부 장관", "산업부 장관", "복지부 장관", "고용부 장관", "여가부 장관",
    "국토부 장관", "해수부 장관", "중기부 장관",
    "국무총리비서실장", "국가안보실장", "대통령비서실장", "국무조정실장"
}

# 제외할 빅그램
excluded_bigrams = {
    "교육 실시", "교육실시", "징역 벌금", "징역벌금", "수립 시행", "수립시행"
}
...
def get_top_words(model, feature_names, n_top_words=4):
    ...
    for topic_idx, topic in enumerate(model.components_):
        top_indices = topic.argsort()[::-1][:n_top_words*2]
        keywords = []
        for i in top_indices:
            word = feature_names[i]
            if word in stopwords or word in excluded_terms or len(word) == 1:
                continue
            if word in excluded_bigrams:
                continue
            parts = word.split()
            if any(part in stopwords or part in excluded_terms for part in parts):
                continue
            if not any(word in k for k in keywords):
...
topic_keywords = get_top_words(lda, feature_names, n_top_words=4)
